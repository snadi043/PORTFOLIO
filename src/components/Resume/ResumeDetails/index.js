...
import responsibilities from '../ResumeComponent/data';
...
const ResumeDetails = () => {
    return(
        <Wrapper className="container-fluid resume-details" style={{display: 'grid', gridTemplateColumns: 'auto auto', rowGap: 'inherit', columnGap: '30px',}}>
            <Timeline
                count={2}
                ...
                responsibilities={responsibilities.education.map((edu, idx) => 
                    edu.map((idvEdu) => <li key={idvEdu}>{idvEdu}</li>)
                )}
            />
            <Timeline
                count={3}
                ...
                responsibilities={responsibilities.work.map((work, idx) =>
                    work.map((idvWork) => <li key={idvWork}>{idvWork}</li>)
                )}
            />
            <Timeline
                count={2}
                ...
                responsibilities={responsibilities.volunteer.map((vol, idx) => 
                    vol.map((idvVol) => <li key={idvVol}>{idvVol}</li>)
                )}
            />
            <Timeline
                count={1}
                ...
                responsibilities={responsibilities.internship.map((intern, idx) => 
                    intern.map((idvIntern) => <li key={idvIntern}>{idvIntern}</li>)
                )}
            />
        </Wrapper>
    );
